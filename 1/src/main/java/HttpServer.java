...
class HttpServer {
    ...
    private void process(Request request, Response response) throws IOException {
        ...
        if (file.isDirectory()) {
            file = new File(root, request.getUri() + "/index12.html");
        }
        ...
        if (file.isFile()) {
            ...
            String eTag = HttpServer.toHexString(md5.digest());

            response.setStatusCode(200);
            response.setHeader("ETag", eTag);
            if (request.headers.containsKey("If-None-Match")) {
                String expectedEtag = request.headers.get("If-None-Match");
            ...
            response.updateBody(fileContents);
    ...
    private static final class Request {
        ...
        private Request(String request) throws UnsupportedEncodingException {
            ...
            uri = URLDecoder.decode(firstLineTokens[1], StandardCharsets.UTF_8.toString());
            headers = new HashMap<>();
    ...
    private static final class Response {
        ...
        private void updateBody(byte[] body) {
            this.setBody(body);
