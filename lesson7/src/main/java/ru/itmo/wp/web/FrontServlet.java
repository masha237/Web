    ...
    private void process(Route route,
        ...
        for (Class<?> clazz = pageClass; actionMethod == null && clazz != null; clazz = clazz.getSuperclass()) {
            for (Method method: clazz.getDeclaredMethods()) {
                if (method.getName().equals(route.getAction()) && Arrays.stream(method.getParameterTypes()).allMatch((param) ->
                        param.equals(HttpServletRequest.class) || param.equals(Map.class)
                )) {
                    actionMethod = method;
                    break;
                }
        ...
        try {
            ...
            actionMethod.invoke(page, Arrays.stream(actionMethod.getParameterTypes()).map((param) -> {
                if(param.equals(HttpServletRequest.class)) {
                    return request;
                } else if (param.equals(Map.class)) {
                    return view;
                } else {
                    throw new RuntimeException("Illegal parameter type '" + param + "'");
                }
            }).toArray());
